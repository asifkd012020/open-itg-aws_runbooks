...
<!-- This overview is to generic I thin, lets keep in line with the other runbooks and add overview related to the service itself-->
## Overview
...
<!-- The Why needs to state what is below, but more towards DynamoDB and why CG would require it. This might be something like, due to the CG standards/controls for cloud resources, we need to enforce least priv... -->
**Why?** 

When you create IAM policies, follow the standard security advice of granting least privilege, or granting only the permissions required to perform a task. Determine what users (and roles) need to do and then craft policies that allow them to perform only those tasks.  
Start with a minimum set of permissions and grant additional permissions as necessary. Doing so is more secure than starting with permissions that are too lenient and then trying to tighten them later. 

<!-- The How needs to be more service specific, like for dynamo db, we need a reader role for those who just need to read data, admin role for admin etc. and examples as below. you can explain that permissions to the dynamoDB allow users access to possible sensitive data, so we need to make sure only the right users have access.-->
**How?** 
...
<!-- Also pretty generic, with service specifics as setting up CloudWatch will be in CloudWatch RB -->
